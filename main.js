...
class LandaSystem {
    ...
    // Initialize cases data (prefers real saved cases from localStorage)
    initializeCases() {
        const saved = (() => {
            try {
                const raw = localStorage.getItem('landaCases');
                const arr = raw ? JSON.parse(raw) : null;
                if (!Array.isArray(arr) || arr.length === 0) return null;

                const normalize = (c) => {
                    const out = { ...c };
                    out.id = out.id || `SF-${Date.now()}`;
                    out.pressType = out.pressType || '';
                    out.pressName = out.pressName || out.press || '';
                    out.system = out.system || '';
                    out.subSystem = out.subSystem || '';
                    out.area = out.area || '';
                    out.subject = out.subject || '';
                    out.description = out.description || '';
                    // Convert textarea steps to array expected by UI
                    if (Array.isArray(out.troubleshootSteps)) {
                        // ok
                    } else if (typeof out.initialSteps === 'string' && out.initialSteps.trim()) {
                        out.troubleshootSteps = out.initialSteps
                            .split(/\r?\n/)
                            .map(s => s.trim())
                            .filter(Boolean);
                    } else {
                        out.troubleshootSteps = [];
                    }

                    // Parts should be string[]
                    if (!Array.isArray(out.parts)) out.parts = [];
                    out.parts = out.parts.map(p => String(p));

                    // Attachments should be number for display
                    if (typeof out.attachments === 'number') {
                        // ok
                    } else if (Array.isArray(out.attachments)) {
                    ...
                    // Rating number
                    out.rating = (typeof out.rating === 'number')
                        ? out.rating
        ...
        // If real cases exist, use them (no demo)
        if (saved) return saved;

        // Fallback demo cases (first run only)
        return [
{
                id: 'SF-2024-001',
                pressType: 'Simplex',
    ...
    filterCases(filters) {
        ...
        return this.cases.filter(case_ => {
            const pressName = case_.pressName || '';
            const system = case_.system || '';
            const status = case_.status || '';
            const subject = (case_.subject || '').toLowerCase();
            const desc = (case_.description || '').toLowerCase();

            const matchesSearch = !f.search || subject.includes(f.search.toLowerCase()) || desc.includes(f.search.toLowerCase());
            const matchesPressType = !f.pressType || case_.pressType === f.pressType;
            const matchesPressName = !f.pressName && !f.press ? true : (pressName === (f.pressName || f.press));
            const matchesSystem = !f.system || system === f.system;
            const matchesStatus = !f.status || status === f.status;

            // Extra filters from dashboard chart clicks
            const matchesRegion = !f.region || (case_.region || '') === f.region;
            const matchesPart = !f.part
                ? true
                : Array.isArray(case_.parts) && case_.parts.some(p => String(p).toLowerCase() === String(f.part).toLowerCase());
