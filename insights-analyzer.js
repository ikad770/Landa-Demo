...
const state = {
  ...
  columnMap: null,
  charts: {},
  visibleSections: new Set(['executive', 'problem', 'insights', 'trends', 'subsystems', 'errors', 'availability', 'versions']),
  filters: {
    dateFrom: '',
    dateTo: '',
    machine: 'all',
...
function cacheDom() {
  [
    ...
    'subsystem-priority-list', 'error-priority-list', 'availability-priority-list', 'duration-insight-list',
    'version-priority-list', 'version-takeaway-list', 'copy-email-summary', 'open-report', 'print-report',
...
function buildSummary(events) {
  ...
  const availabilityLeaders = [...machines].sort((a, b) => b.totalDowntime - a.totalDowntime).slice(0, 5);
  const versionAnalysis = buildVersionAnalysis(events);
  const smartInsights = buildSmartInsights(events, machines, topErrors, topSubsystems, versionAnalysis);
  const recommendedFocus = buildRecommendedFocus(machines, smartInsights, topErrors, topSubsystems, availabilityLeaders);
...
function buildMachineSummary(machine, events) {
  ...
  const trendDirection = worseningFlag ? 'Worsening' : (recent.length + 4 < older.length ? 'Improving' : 'Stable');
...
function buildSmartInsights(events, machines, topErrors, topSubsystems, versionAnalysis) {
  const insights = [];
  ...
  if (multiMachineError) {
    insights.push({
      ...
      focus: `Contain ${multiMachineError.key} and compare the affected machines for a shared cause.`,
      priority: 100
    });
  }
  ...
  if (overloadedSubsystem) {
    insights.push({
      ...
      focus: `Inspect ${overloadedSubsystem.key} first and compare diagnostics across affected presses.`,
      priority: 90
    });
  }
  ...
  if (highDowntimeLowErrors) {
    insights.push({
      ...
      focus: `Prioritize availability restoration on ${highDowntimeLowErrors.machine} before chasing alert count alone.`,
      priority: 85
    });
  }
  ...
  if (worseningMachines.length) {
    insights.push({
      ...
      focus: 'Review the most recent events first and stop the acceleration before the next production window.',
      priority: 80
    });
  }
  ...
  if (concentratedMachine) {
    insights.push({
      ...
      focus: `Treat ${concentratedMachine.topSubsystem} as the likely root-cause area instead of a broad machine issue.`,
      priority: 76
    });
  }

  const hiddenRiskMachine = machines.find((machine) => machine.totalErrors < 50 && machine.repeatedErrorFlag);
  if (hiddenRiskMachine) {
    insights.push({
      title: 'Hidden risk machine with moderate total volume',
      explanation: `${hiddenRiskMachine.machine} is still below fleet severity thresholds overall, but one recurring error dominates its event profile and may escalate quickly.`,
      entities: [{ type: 'machine', value: hiddenRiskMachine.machine }, { type: 'error', value: hiddenRiskMachine.topError }],
      focus: `Investigate ${hiddenRiskMachine.topError} before this machine graduates into a higher severity state.`,
      priority: 74
    });
  }

  const noisyVersion = versionAnalysis.find((row) => row.machineCount >= 2 && row.errorPerMachine >= 80);
  if (noisyVersion) {
    insights.push({
      title: 'Version-linked issue concentration',
      explanation: `Version ${noisyVersion.version} shows the highest issue density with ${noisyVersion.errorPerMachine} average errors per machine across ${noisyVersion.machineCount} machines.`,
      entities: noisyVersion.machines.slice(0, 4).map((machine) => ({ type: 'machine', value: machine })),
      focus: `Compare the software build history of version ${noisyVersion.version} against healthier groups.`,
      priority: 70
    });
  }
  ...
  if (!insights.length) {
    insights.push({
      ...
      entities: [],
      focus: 'Continue monitoring the current fleet slice and use drill-down only for local investigations.',
      priority: 10
    });
  }
...
function buildRecommendedFocus(machines, insights, topErrors, topSubsystems, availabilityLeaders) {
  const focus = [];
  ...
  if (availabilityLeaders[0]) focus.push({ title: `Reduce downtime on ${availabilityLeaders[0].machine}`, text: `${formatMinutes(availabilityLeaders[0].totalDowntime)} lost in the filtered range.`, type: 'machine', value: availabilityLeaders[0].machine });
  if (insights[0]) focus.push({ title: 'Follow top smart insight', text: insights[0].focus || insights[0].title, type: insights[0].entities[0]?.type || 'machine', value: insights[0].entities[0]?.value || machines[0]?.machine });
...
function buildAiSummary(machines, statusCounts, topErrors, topSubsystems) {
  ...
  return `Out of ${machines.length} machines, ${criticalCount} are classified as Critical and ${dominantStatus[1]} machines fall into the dominant ${dominantStatus[0]} state, mainly driven by ${mainSubsystem} issues and repeat ${mainError} alerts. Prioritize the top flagged machines first, then use drill-down to confirm recurrence, duration impact, and recovery behavior.`;
}
...
function renderDashboard() {
  ...
  renderSmartInsights();
  renderPriorityLists();
  renderAvailabilityAndVersionLists();
  renderCharts();
...
function renderProblemMachines() {
  ...
  els.problemMachines.innerHTML = machines.length ? machines.map((machine) => `
    <button class="machine-card" data-machine="${escapeHtml(machine.machine)}">
      ...
      <div class="machine-reason mt-2">Trend: ${machine.trendDirection}</div>
...
function renderSmartInsights() {
  els.smartInsights.innerHTML = state.summary.smartInsights.map((insight) => `
    <div class="insight-card">
      ...
      <div class="insight-text" style="margin-top:.55rem; color:#dbeafe;"><strong>Focus:</strong> ${escapeHtml(insight.focus || 'Review the affected entities and validate recurrence.')}</div>
...
function renderAvailabilityAndVersionLists() {
  els.availabilityPriorityList.innerHTML = state.summary.availabilityLeaders.map((machine) => `
    <div class="mini-row">
      <div>
        <button data-entity-type="machine" data-entity-value="${escapeHtml(machine.machine)}">${escapeHtml(machine.machine)}</button>
        <small>${machine.totalErrors} errors · ${machine.trendDirection} trend</small>
      </div>
      <span class="mini-value">${formatMinutes(machine.totalDowntime)}</span>
    </div>
  `).join('');

  els.durationInsightList.innerHTML = state.summary.availabilityLeaders.map((machine) => `
    <div class="mini-row">
      <div>
        <button data-entity-type="machine" data-entity-value="${escapeHtml(machine.machine)}">${escapeHtml(machine.machine)}</button>
        <small>${machine.highDowntimeFlag ? 'High availability impact' : 'Duration needs monitoring'}</small>
      </div>
      <span class="mini-value">${Math.round(machine.avgDuration)}m avg</span>
    </div>
  `).join('');

  els.versionPriorityList.innerHTML = state.summary.versionAnalysis.map((version) => `
    <div class="mini-row">
      <div>
        ${version.machines[0] ? `<button data-entity-type="machine" data-entity-value="${escapeHtml(version.machines[0])}">${escapeHtml(version.version)}</button>` : `<strong>${escapeHtml(version.version)}</strong>`}
        <small>${version.machineCount} machines · ${version.totalErrors} total errors</small>
      </div>
      <span class="mini-value">${version.errorPerMachine}</span>
    </div>
  `).join('') || '<div class="section-subtitle">No version signal was found in the current slice.</div>';

  els.versionTakeawayList.innerHTML = state.summary.versionAnalysis.slice(0, 4).map((version) => `
    <div class="mini-row">
      <div>
        <strong>${escapeHtml(version.version)}</strong>
        <small>${version.machineCount} machines · ${formatMinutes(version.totalDowntime)} downtime</small>
      </div>
      <span class="mini-value">${version.errorPerMachine}/machine</span>
    </div>
  `).join('') || '<div class="section-subtitle">No version analysis available.</div>';

  bindEntityButtons(els.availabilityPriorityList);
  bindEntityButtons(els.durationInsightList);
  bindEntityButtons(els.versionPriorityList);
}

function buildVersionAnalysis(events) {
  return topCounts(events, (event) => event.version, 6).map((row) => {
    const versionEvents = events.filter((event) => event.version === row.key);
    const machines = uniqueSorted(versionEvents.map((event) => event.machine));
    return {
      version: row.key,
      totalErrors: row.value,
      totalDowntime: sum(versionEvents, (event) => event.durationMinutes),
      machineCount: machines.length,
      machines,
      errorPerMachine: machines.length ? Math.round(row.value / machines.length) : row.value
    };
  });
}
      ...
      <div class="details-shell" style="margin-top:1rem;">
        ...
        <div class="glass-card">
          ...
          <ul class="label-list">
            ...
            <li>Trend direction: ${machine.trendDirection}</li>
          </ul>
        </div>
      </div>
      <div class="details-shell" style="margin-top:1rem;">
        <div class="glass-card">
          <div class="section-title">Recovery & duration profile</div>
          <ul class="label-list">
            <li>Average duration: ${Math.round(machine.avgDuration)} minutes</li>
            <li>Total downtime: ${formatMinutes(machine.totalDowntime)}</li>
            <li>Recovery loop detected: ${machine.repeatedRecoveryFlag ? 'Yes' : 'No'}</li>
            <li>Concentrated failure source: ${machine.concentratedFailureFlag ? 'Yes' : 'No'}</li>
          </ul>
        </div>
        <div class="glass-card">
          <div class="section-title">Recent critical events</div>
          <ul class="label-list">${machine.events.filter((event) => /critical/i.test(event.errorState)).slice(-5).reverse().map((event) => `<li>${formatDateTime(event.startTime)} · ${escapeHtml(event.title)} · ${formatMinutes(event.durationMinutes)}</li>`).join('') || '<li>No recent critical-state events.</li>'}</ul>
        </div>
      </div>
...
function buildEmailSummaryText() {
  ...
  return [
    ...
    '',
    'Critical Machines:',
    ...(focusMachines.length ? focusMachines.map((machine) => `- ${machine.machine} → recurring ${machine.topSubsystem} issues (${machine.totalErrors} errors, ${formatMinutes(machine.totalDowntime)} downtime, ${machine.trendDirection.toLowerCase()} trend)`) : ['- None in the current filter slice']),
    '',
    'Attention Points:',
